...
def loss_fn(
    ...
    if normalize_per_patch:
        ...
        pred_patches = (pred_patches - mean) / (var + eps).sqrt()
    
    # Compute MSE per patch
    loss_per_patch: Float[Tensor, "B N"] = (pred_patches - image_patches).pow(2).mean(dim=-1)
    
    # Average loss over all masked patches
    num_masked = mask.sum()
    if num_masked > 0:
        loss = (loss_per_patch * mask).sum() / num_masked
    else:
...
def train(
    ...
    for epoch in range(cfg.train.num_epochs):
        ...
        for idx, batch in enumerate(train_dataloader):
            ...
            loss = loss_fn(image, pred, mask, patch_stride=decoder.module.patch_stride) / cfg.train.grad_accum
        ...
        for idx, batch in enumerate(val_dataloader):
            ...
            with (
                ...
                pred = decoder(logits, mask)
            pred = pred.to(dtype=torch.float32)
            val_loss = loss_fn(image, pred, mask, patch_stride=decoder.module.patch_stride)
                ...
                wandb.log(
                    {
                        ...
                        "epoch/pred_vs_image": make_wandb_recon_pair(pred, image),  # pyrefly:ignore
                    }
                )
